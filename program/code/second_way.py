...
def new_ARE(id,total):
    tmp = 0
    tmp = sketch_query(store_set[id],id)
    return tmp/total 
def which_switch(id):
    MIN = 30000
    which = -1
...
for packets in pcaps:
    try:
        if(packets['IP'].src in flow_set):
            ...
            sketch_add(store_set[packets['IP'].src],packets['IP'].src)
        else:
            flow_set[packets['IP'].src] = 1
            tmp = which_switch(packets['IP'].src)
            sketch_add(tmp,packets['IP'].src)

            store_set[packets['IP'].src] = tmp
    except IndexError:
        pass
...
for value in flow_set.keys():
    if(big_are<new_ARE(value,flow_set[value])):
        big_are = new_ARE(value,flow_set[value])
        who = value
print(who,big_are)
print(store_set[who])
